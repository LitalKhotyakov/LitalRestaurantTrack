...
class RestaurantDetailViewController: UIViewController, UITableViewDataSource, UITableViewDelegate,
  ...
  override func viewDidLoad() {
    ...
    let query = restaurantReference!.collection("ratings").order(by: "date", descending: true)
    print("🔍 Loading reviews for restaurant: \(restaurantReference!.path)")
    print("📍 Restaurant ID: \(restaurantReference!.documentID)")
    print("🏪 Restaurant name: \(restaurant?.name ?? "Unknown")")
    print("🔗 Full query path: \(query)")
    localCollection = LocalCollection(query: query) { [unowned self] changes in
      ...
      print("📊 Loaded \(self.localCollection.count) reviews for restaurant: \(self.restaurantReference!.path)")
      for (index, review) in self.localCollection.items.enumerated() {
        print("  Review \(index): \(review.username) - \(review.text) (\(review.rating) stars)")
      }
      
      // Verify that all reviews belong to this restaurant
      let expectedRestaurantPath = self.restaurantReference!.path
      let allReviewsBelongToRestaurant = self.localCollection.documents.allSatisfy { doc in
        doc.reference.path.hasPrefix(expectedRestaurantPath)
      }
      print("✅ All reviews belong to restaurant: \(allReviewsBelongToRestaurant)")
      
      // Reload the entire table view to handle all changes properly
      // This prevents duplication issues when the collection is updated
  ...
  override func viewWillDisappear(_ animated: Bool) {
    ...
    // Stop listening when view disappears to prevent memory leaks
    localCollection.stopListening()
  }
